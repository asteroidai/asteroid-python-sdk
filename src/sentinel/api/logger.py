...
from sentinel.api.generated.sentinel_api_client.api.run.create_new_chat import (
...
from sentinel.supervision.config import (
...
class APILogger:
    """Handles logging to the Sentinel API, including supervision and resampling."""
    ...
    def log_request(self, request_data: Dict[str, Any], run_id: UUID) -> None:
        """
        Log the request data. Currently a no-op as the Sentinel API doesn't require request data
    ...
    def log_response(
        ...
        try:
            ...
            # Send the request to Sentinel API
    ...
    def send_api_request(self, run_id: UUID, body: SentinelChat) -> Any:
        """
        Send the API request to the Sentinel API and handle the response.
